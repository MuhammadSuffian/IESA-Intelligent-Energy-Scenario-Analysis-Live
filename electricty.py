...
def electricty_dashboard():
    # Fetch Annual Electricity Data
    electricity_data = fetch_table_data("annual_electricity_data")

    # Fetch Sector-wise Electricity Consumption Data
    sector_data = fetch_table_data("electricity_consumption_by_sector_gwh")

    # Fetch Province-wise Electricity Consumption Data
    province_data = fetch_table_data("province_wise_electricity_consumption_gwh")
    ...
    if not electricity_data.empty and not sector_data.empty:
        ...
        st.markdown(
            "<div class='section-header'><h2 style='margin:0;'>⚡ Electricity Overview</h2></div>",
            unsafe_allow_html=True,
        )

        if len(electricity_columns) > 4:
            latest_year = electricity_data[electricity_columns[0]].iloc[-1]
            latest_data = electricity_data[
                electricity_data[electricity_columns[0]] == latest_year
            ]
            ...
            utilization_rate = (
                (generation / installed_capacity * 100) if installed_capacity > 0 else 0
            )
            import_percentage = (
                (imports / (generation + imports) * 100)
                if (generation + imports) > 0
                else 0
            )
            consumption_rate = (
                (consumption / (generation + imports) * 100)
                if (generation + imports) > 0
                else 0
            )
            ...
            if len(electricity_columns) > 2:
                chart2 = alt.Chart(electricity_data).mark_bar(color="#2E8B57").encode(
                    ...
                    tooltip=[
                        alt.Tooltip(electricity_columns[0], title="Year"),
                        alt.Tooltip(
                            electricity_columns[2],
                            title="Generation (GWh)",
                            format=",.1f",
                        ),
                    ],
                ).properties(title="Electricity Generation")
                trend_line = alt.Chart(electricity_data).mark_line(
                    color="red", strokeDash=[5, 5]
                ).encode(
                    x=electricity_columns[0],
                    y=f"mean({electricity_columns[2]})",
                )
                st.altair_chart((chart2 + trend_line).interactive(), use_container_width=True)
            else:
                st.warning("Missing Electricity Generation Data")

        with col3:
            if len(electricity_columns) > 3:
                chart3 = alt.Chart(electricity_data).mark_area(
                    ...
                    tooltip=[
                        alt.Tooltip(electricity_columns[0], title="Year"),
                        alt.Tooltip(
                            electricity_columns[3], title="Import (GWh)", format=",.1f"
                        ),
                    ],
                ).properties(title="Electricity Import")
                st.altair_chart(chart3.interactive(), use_container_width=True)
        ...
        with col3:
            if len(electricity_columns) > 4:
                latest_data = electricity_data.iloc[-1]
                fig = go.Figure(
                    data=[
                        go.Pie(
                            labels=["Generation", "Import", "Consumption"],
        ...
        st.markdown("<div class='line-chart-container'>", unsafe_allow_html=True)
        st.markdown(
            "<h3 class='chart-title'>Electricity Trends Over Time</h3>",
            unsafe_allow_html=True,
        )
        ...
        if len(electricity_columns) > 1:
            fig = px.line(
                ...
                x=electricity_columns[0],
                y=[
                    electricity_columns[1],
                    electricity_columns[2],
                    electricity_columns[3],
                    electricity_columns[4],
                ],
                title="Electricity Metrics Trend Analysis",
                labels={
        ...
        st.markdown(
            "<div class='section-header'><h2 style='margin:0;'>🏢 Sector-wise Electricity Consumption</h2></div>",
            unsafe_allow_html=True,
        )
        st.markdown("<div class='chart-container'>", unsafe_allow_html=True)
        ...
        with sec_col1:
            if len(sector_columns) > 1:
                chart_data = sector_data.rename(
                    columns={sector_columns[0]: "Year", sector_columns[1]: "Residential"}
                )
                fig = px.line(chart_data, x="Year", y="Residential", title="Residential Consumption", markers=True)
                fig.update_traces(line=dict(color="#8A2BE2", width=3))
                fig.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=40, r=20, t=50, b=40))
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Missing Residential Data")
        ...
        with sec_col2:
            if len(sector_columns) > 2:
                chart_data = sector_data.rename(
                    columns={sector_columns[0]: "Year", sector_columns[2]: "Commercial"}
                )
                fig = px.line(chart_data, x="Year", y="Commercial", title="Commercial Consumption", markers=True)
                fig.update_traces(line=dict(color="#1E90FF", width=3))
                fig.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=40, r=20, t=50, b=40))
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        with sec_col3:
            if len(sector_columns) > 3:
                chart_data = sector_data.rename(
                    columns={sector_columns[0]: "Year", sector_columns[3]: "Industrial"}
                )
                fig = px.line(chart_data, x="Year", y="Industrial", title="Industrial Consumption", markers=True)
                fig.update_traces(line=dict(color="#32CD32", width=3))
                fig.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=40, r=20, t=50, b=40))
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        with sec_col3:
            if len(sector_columns) > 9:
                ...
                latest_data = sector_data[sector_data[sector_columns[0]] == latest_year]
                sectors = [
                    sector_columns[1], sector_columns[2], sector_columns[3],
                    sector_columns[4], sector_columns[5], sector_columns[7], sector_columns[8],
                ]
                sector_values = [latest_data[col].values[0] for col in sectors]
                sector_names = ["Residential", "Commercial", "Industrial", "Agricultural",
                                "Street Light", "Bulk Supply", "Other Govt"]
                fig = go.Figure(
                    data=[go.Pie(
                        labels=sector_names, values=sector_values,
                        hole=0.3, marker_colors=px.colors.qualitative.Set2,
                    )]
                )
                fig.update_layout(
                    title_text=f"Consumption by Sector ({latest_year})",
        ...
        # Sector trend & stacked area
        st.markdown("<div class='line-chart-container'>", unsafe_allow_html=True)
        st.markdown(
            "<h3 class='chart-title'>Sector-wise Consumption Comparison</h3>",
            unsafe_allow_html=True,
        )
        ...
        if len(sector_columns) > 1:
            fig = px.line(
                sector_data,
                x=sector_columns[0],
                y=[sector_columns[1], sector_columns[2], sector_columns[3],
                   sector_columns[4], sector_columns[5], sector_columns[7], sector_columns[8]],
                title="Sector-wise Electricity Consumption Trends",
                labels={sector_columns[0]: "Year", "value": "Consumption (GWh)", "variable": "Sector"},
                color_discrete_sequence=px.colors.qualitative.Plotly,
            )
            fig.update_layout(
                xaxis_title="Year", yaxis_title="Consumption (GWh)",
                legend_title="Sector", hovermode="x unified", height=450,
        ...
        st.markdown(
            "<div class='section-header'><h2 style='margin:0;'>🗺️ Province-wise Electricity Consumption</h2></div>",
            unsafe_allow_html=True,
        )
        st.markdown("<div class='chart-container'>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='line-chart-container'>", unsafe_allow_html=True)
        st.markdown(
            "<h3 class='chart-title'>Provincial Consumption Comparison</h3>",
            unsafe_allow_html=True,
        )
        ...
        if len(province_columns) > 1:
            fig = px.line(
                ...
                y=[province_columns[1], province_columns[2], province_columns[3],
                   province_columns[4], province_columns[5], province_columns[6]],
                title="Province-wise Electricity Consumption Trends",
                labels={province_columns[0]: "Year", "value": "Consumption (GWh)", "variable": "Province"},
                color_discrete_sequence=px.colors.qualitative.Dark24,
            )
            fig.update_layout(
                xaxis_title="Year", yaxis_title="Consumption (GWh)",
                legend_title="Province", hovermode="x unified", height=450,
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            )
            st.plotly_chart(fig, use_container_width=True)
            ...
            latest_year = province_data[province_columns[0]].iloc[-1]
            latest_data = province_data[province_data[province_columns[0]] == latest_year]
            provinces = [province_columns[1], province_columns[2], province_columns[3],
                         province_columns[4], province_columns[5]]
            province_values = [latest_data[col].values[0] for col in provinces]
            province_names = ["Punjab", "Sindh", "KPK", "Balochistan", "AJK"]
