...
#         st.markdown(
#             "© 2026 Integrated Energy System Analytics Dashboard. All rights reserved." 
#             " | Version 1.0", 
#             unsafe_allow_html=True
#         )
#         st.markdown("</div>", unsafe_allow_html=True)
...
def fetch_table_data(table_name: str) -> pd.DataFrame:
    ...
    Replaces the MySQL fetch_table_data() from mysql_con.
    """
    try:
...
def load_dash(logger):
    ...
    if selected == "Dashboard Home":
        ...
        # ── Fetch key metrics via Supabase REST ───────────────────────────────
        try:
            supabase_url, api_key = get_supabase_creds()
            headers = {
                **_supabase_headers(api_key),
            ...
            # Total primary energy supply (sum)
            resp = requests.get(
                f"{supabase_url}/rest/v1/energy_supply_and_consumption_analysis",
                headers=headers,
                params={"select": "total_primary_energy_supply_mtoe"},
                timeout=20,
            )
            resp.raise_for_status()
            total_production = sum(
                row["total_primary_energy_supply_mtoe"] or 0 for row in resp.json()
            )

            # Total final consumption (sum)
            resp = requests.get(
                f"{supabase_url}/rest/v1/energy_supply_and_consumption_analysis",
                headers=headers,
                params={"select": "total_final_consumption_energy_mtoe"},
                timeout=20,
            )
            resp.raise_for_status()
            total_consumption = sum(
                row["total_final_consumption_energy_mtoe"] or 0 for row in resp.json()
            )

            # Electricity generation (sum)
            resp = requests.get(
                f"{supabase_url}/rest/v1/annual_electricity_data",
                headers=headers,
                params={"select": "generation_gwh"},
                timeout=20,
            )
            resp.raise_for_status()
            electricity_production = sum(
                row["generation_gwh"] or 0 for row in resp.json()
            )

            # Natural gas production (sum)
            resp = requests.get(
                f"{supabase_url}/rest/v1/natural_gas_production_and_consumption",
                headers=headers,
                params={"select": "natural_gas_production"},
                timeout=20,
            )
            resp.raise_for_status()
            gas_production = sum(
                row["natural_gas_production"] or 0 for row in resp.json()
            )

            logger.info(
                f"Fetched metrics: production={total_production}, "
                f"consumption={total_consumption}, "
                f"electricity={electricity_production}, "
                f"gas={gas_production}"
            )
        except Exception as e:
            logger.error(f"Error fetching dashboard metrics: {e}")
            # Fallback data if the API query fails
            total_production      = 85.7
            total_consumption     = 79.3
            electricity_production = 41.2
            gas_production        = 33.5
        ...
        try:
            # ── Fetch energy_by_source via Supabase REST ──────────────────────
            energy_mix = fetch_table_data("energy_by_source")
            logger.info(f"Fetched energy mix data: {energy_mix.shape[0]} rows")
        ...
        try:
            # ── Fetch total_energy via Supabase REST ──────────────────────────
            energy_trend = fetch_table_data("total_energy")
            logger.info(f"Fetched energy trend data: {energy_trend.shape[0]} rows")
