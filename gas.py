...
# Fetch data from a Supabase table via PostgREST
    ...
    try:
        ...
        headers = {
            **_supabase_headers(api_key),
            # Lift the default 1 000-row cap
            "Range-Unit": "items",
            "Range": "0-",
        }
        ...
        # Convert numeric columns (all except the first) to float
        for col in data.columns[1:]:
            data[col] = pd.to_numeric(data[col], errors="coerce")
        data.fillna(0, inplace=True)
...
def gas_dashboard():
    ...
    .metric-label  { color: #555; font-size: 1rem; font-weight: 500; }
    .metric-trend  { font-size: 0.9rem; margin-top: 5px; }
    .metric-trend.positive { color: #2ECC40; }
    .metric-trend.negative { color: #FF4136; }
    .circular-progress-container {
        display: flex;
        flex-direction: column;
        align-items: center;
    ...
    .circular-progress-value { font-size: 1.5rem; font-weight: bold; color: #333; }
    .circular-progress-label { margin-top: 10px; font-size: 1rem; color: #555; font-weight: 500; }
    .custom-divider {
        height: 3px;
        background: linear-gradient(90deg, rgba(16,100,102,0.2), rgba(16,100,102,0.8), rgba(16,100,102,0.2));
        margin: 30px 0;
    ...
    ::-webkit-scrollbar-thumb { background: #106466; border-radius: 10px; }
    ::-webkit-scrollbar-thumb:hover { background: #329D9C; }
    </style>
    """, unsafe_allow_html=True)
    ...
    if not gas_data.empty:
        ...
        # Consumption
        trend_class = "positive" if consumption_trend <= 0 else "negative"
        trend_icon  = "↓" if consumption_trend <= 0 else "↑"
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-label">Gas Consumption</div>
        ...
        with col1:
            fig = create_gauge_chart(
                ...
                0, 150,
                {
                    'low':    {'color': '#2ECC40', 'range': [0,   70]},
                    'medium': {'color': '#FFDC00', 'range': [70,  100]},
                    'high':   {'color': '#FF4136', 'range': [100, 150]},
                },
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col2:
            fig2 = create_gauge_chart(
                ...
                0, 200,
                {
                    'low':    {'color': '#FF4136', 'range': [0,   100]},
                    'medium': {'color': '#FFDC00', 'range': [100, 150]},
                    'high':   {'color': '#2ECC40', 'range': [150, 200]},
                },
            )
            st.plotly_chart(fig2, use_container_width=True)
        ...
        with col1:
            fig3 = px.line(
                filtered_data, x=gas_columns[0], y=gas_columns[1],
                title="Gas Production Trend Over Time",
                markers=True, color_discrete_sequence=["#106466"],
            )
            fig3.add_trace(go.Scatter(
                x=filtered_data[gas_columns[0]], y=filtered_data[gas_columns[1]],
                mode='lines', line=dict(width=0),
                fillcolor='rgba(16, 100, 102, 0.2)', fill='tozeroy', showlegend=False,
            ))
            fig3.update_layout(
                xaxis_title="Year", yaxis_title="Production (Bcf)",
                hovermode="x unified",
        ...
        with col2:
            fig4 = px.line(
                filtered_data, x=gas_columns[0], y=gas_columns[2],
                title="Gas Consumption Trend Over Time",
                markers=True, color_discrete_sequence=["#A30000"],
            )
            fig4.add_trace(go.Scatter(
                x=filtered_data[gas_columns[0]], y=filtered_data[gas_columns[2]],
                mode='lines', line=dict(width=0),
                fillcolor='rgba(163, 0, 0, 0.2)', fill='tozeroy', showlegend=False,
            ))
            fig4.update_layout(
                xaxis_title="Year", yaxis_title="Consumption (Bcf)",
                hovermode="x unified",
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            )
            fig4.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(230,230,230,0.8)')
        ...
        fig5.add_trace(go.Scatter(
            ...
            line=dict(color='#106466', width=3), marker=dict(size=8),
        ))
        fig5.add_trace(go.Scatter(
            x=filtered_data[gas_columns[0]], y=filtered_data[gas_columns[2]],
            mode='lines+markers', name='Consumption',
            line=dict(color='#A30000', width=3), marker=dict(size=8),
        ))

        y_surplus = [
            filtered_data[gas_columns[1]].iloc[i] - filtered_data[gas_columns[2]].iloc[i]
            for i in range(len(filtered_data))
        ]
        fig5.add_trace(go.Bar(
            x=filtered_data[gas_columns[0]], y=y_surplus,
            name='Surplus/Deficit',
            marker=dict(color=['#2ECC40' if v >= 0 else '#FF4136' for v in y_surplus]),
            opacity=0.6,
        ))
        fig5.update_layout(
            title="Production vs. Consumption with Surplus/Deficit",
            xaxis_title="Year", yaxis_title="Natural Gas (Bcf)",
            legend_title="Metrics", hovermode="x unified", barmode='relative',
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        )
        fig5.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(230,230,230,0.8)')
        fig5.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(230,230,230,0.8)')
        ...
        with col1:
            fig6 = px.bar(
                filtered_data, x=gas_columns[0], y=gas_columns[3],
                title="Gas Revenue Over Time",
                color_discrete_sequence=['#3CB371'],
            )
            fig6.update_layout(
                xaxis_title="Year", yaxis_title="Revenue (PKR Billions)",
                yaxis=dict(range=[0, 2000]),
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            )
            fig6.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(230,230,230,0.8)')
        ...
        with col2:
            fig7 = px.bar(
                filtered_data, x=gas_columns[0], y=gas_columns[4],
                title="Gas Import Costs Over Time",
                color_discrete_sequence=['#FF8C00'],
            )
            fig7.update_layout(
                xaxis_title="Year", yaxis_title="Import Costs (PKR Billions)",
                yaxis=dict(range=[0, 2000]),
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            )
            fig7.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(230,230,230,0.8)')
        ...
        fig8.update_layout(
            title="Yearly Financial Balance (Revenue - Import Costs)",
            xaxis_title="Year", yaxis_title="Net Balance (PKR Billions)",
            waterfallgap=0.2,
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        )
        st.plotly_chart(fig8, use_container_width=True)

        # Revenue vs import costs comparison
        fig9 = go.Figure()
        fig9.add_trace(go.Scatter(
            x=filtered_data[gas_columns[0]], y=filtered_data[gas_columns[3]],
            mode='lines+markers', name='Revenue',
            line=dict(color='#3CB371', width=3), marker=dict(size=8),
        ))
        fig9.add_trace(go.Scatter(
            x=filtered_data[gas_columns[0]], y=filtered_data[gas_columns[4]],
            mode='lines+markers', name='Import Costs',
            line=dict(color='#FF8C00', width=3), marker=dict(size=8),
        ))
        fig9.add_trace(go.Scatter(
            x=filtered_data[gas_columns[0]], y=filtered_data["Net Balance"],
            mode='lines', name='Profit/Loss',
            line=dict(color='#106466', width=2, dash='dot'),
            fill='tozeroy', fillcolor='rgba(16, 100, 102, 0.2)',
        ))
        fig9.update_layout(
            title="Revenue vs. Import Costs with Profit/Loss",
            xaxis_title="Year", yaxis_title="Financial Value (PKR Billions)",
            legend_title="Financial Metrics", hovermode="x unified",
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        )
        st.plotly_chart(fig9, use_container_width=True)
