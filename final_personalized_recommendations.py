...
# def load_personalized_recommendations(logger):
...
#         db_api_key=st.secrets.get("db_api_key", "").strip()
#     except Exception:
#         api_key = None
#         db_api_key=None
...
#     if user_input:
#         logger.info(f"User sent chat message: {user_input}")
...
#         # After user sends a chat message
...
#             logger.info(f"Sidebar button clicked: {btn} for table {selected_table}")
#             user_input = f"{selected_table} - {btn}"
#             st.session_state.messages.append({"role": "user", "content": user_input})
#             with st.spinner("Thinking..."):
#                 bot_reply = asyncio.run(get_response(user_input))
#             st.session_state.messages.append({"role": "assistant", "content": bot_reply})
#             st.toast(f"{btn} action triggered!", icon="⚡")
#             st.rerun()
...
#         logger.info(f"Scenario category selected: {selected_category}")
...
#             logger.info(f"Scenario selected: {selected_scenario}")
#             if st.sidebar.button("Generate Recommendation", key=f"rec_{selected_scenario}"):
#                 logger.info(f"Generate Recommendation for scenario: {selected_scenario}")
...
#                 logger.info(f"Analyze Trends for scenario: {selected_scenario}")
...
#                 # After scenario actions
#                 st.toast("Trends analyzed!", icon="📈")

#             if st.sidebar.button("Generate Report", key=f"report_{selected_scenario}"):
#                 logger.info(f"Generate Report for scenario: {selected_scenario}")
...
def load_personalized_recommendations(logger):
    ...
    def fetch_tables() -> list[str]:
        """
        Return user-created table names from the public schema via PostgREST RPC.
    ...
    def fetch_scenario_categories() -> list[str]:
        """
        Fetch distinct scenario categories from the scenario_definitions table.
        """
        try:
            url = f"{SUPABASE_URL}/rest/v1/scenario_definitions"
            headers = {
                **_supabase_headers(),
                "Range-Unit": "items",
                "Range": "0-",
            ...
            resp.raise_for_status()
            rows = resp.json()
            categories = list({row["category"] for row in rows if row.get("category")})
            categories.sort()
            return categories
        except Exception as e:
            logger.error(f"Error fetching scenario categories: {e}")
            st.error(f"Error fetching scenario categories: {e}")
            return []

    def fetch_scenarios(category: str) -> list[str]:
        """
        Fetch scenarios for a given category from the scenario_definitions table.
        """
        try:
            url = f"{SUPABASE_URL}/rest/v1/scenario_definitions"
            headers = {
            ...
            resp = requests.get(
                url, headers=headers,
                params={"select": "scenario", "category": f"eq.{category}"},
                timeout=15
            )
            resp.raise_for_status()
            scenarios = [row["scenario"] for row in resp.json() if row.get("scenario")]
            return scenarios
        except Exception as e:
            logger.error(f"Error fetching scenarios for category '{category}': {e}")
            st.error(f"Error fetching scenarios: {e}")
            return []
    ...
    st.markdown(
        ...
        .user-container { display: flex; align-content: flex-end; justify-content: flex-end; align-items: flex-end; font-size: 15px; }
        .bot-container { font-size: 15px; }
        .user-msg {
            width: fit-content;
            background-color: #e9f5ff;
            color: #222;
...
# TEMPORARY DEBUG — remove after confirming keys are visible
    ...
    st.sidebar.markdown("<p style='color:white; font-size:24px; font-weight:600; margin:4px 0 10px 0; letter-spacing:0.3px;'>Wisdom Mining</p>", unsafe_allow_html=True)
    ...
    def create_recommendation_report(messages=None):
        ...
        if messages and len(messages) > 0:
            ...
            for i, message in enumerate(messages):
                if message["role"] == "user":
                    c.setFillColor(colors.HexColor("#136a8a"))
                    icon = "🧑"
                    prefix = "User: "
                else:
                    c.setFillColor(colors.HexColor("#4AC29A"))
                    icon = "🤖"
                    prefix = "IESA Assistant: "

                c.setFont("Helvetica-Bold", 12)
                c.drawString(40, content_y, f"{icon} {prefix}")
                content_y -= 20

                bubble_color = (
                    colors.HexColor("#e9f5ff") if message["role"] == "user"
                    else colors.HexColor("#f0f8f1")
                )
                ...
                for paragraph in message["content"].split('\n'):
                    if not paragraph.strip():
                        continue
                    wrapped_text = wrap(paragraph, width=75)
                    bubble_height = len(wrapped_text) * 15 + 10
                    ...
                    c.setFillColor(bubble_color)
                    if message["role"] == "user":
                        bubble_x, bubble_width = 120, width - 160
                    else:
                        bubble_x, bubble_width = 60, width - 100
                    c.roundRect(bubble_x, content_y - bubble_height, bubble_width, bubble_height, 8, fill=1, stroke=1)
                    ...
                    text_y = content_y - 15
                    for line in wrapped_text:
                        c.drawString(bubble_x + 10, text_y, line)
                        text_y -= 15
                    content_y -= bubble_height + 20
