...
# def load_prediction_engine(logger):
...
def fetch_tables() -> list:
    try:
        ...
        return [
            ...
            "province_wise_energy_consumption",
            "provincial_energy_distribution_and_transmission_losses",
            "scenario_definitions",
            "sector_wise_energy_consumption",
            "total_imports_lng",
            "total_proved_reserves",
        ]
...
def load_prediction_engine(logger):
    ...
    def create_prediction_report(chart_paths=None):
        ...
        algo_descriptions = {
            "Linear":        "Simple and fast model assuming a linear relationship between variables.",
            "Polynomial":    "Captures non-linear relationships by introducing polynomial terms.",
            "Random Forest": "Ensemble method using multiple decision trees for robust predictions.",
            "SVR":           "Support Vector Regression uses kernel functions to model complex relationships.",
        }
    ...
    if selected_table:
        ...
        if data.empty:
            st.warning("⚠️ No data returned for the selected table.")
            st.stop()
    ...
    if st.session_state.get("results"):
        ...
        for index, result in enumerate(st.session_state.results):
            with (col1 if index % 2 == 0 else col2):
                st.markdown(
                    f"<div class='chart-title'>{result['type']} Regression</div>",
                    unsafe_allow_html=True,
                )

                axis_cfg = dict(
                    labelColor='black', labelFontSize=14, labelFontWeight='normal',
                    titleColor='black', titleFontSize=16, titleFontWeight='normal',
                )

                actual_chart = (
                    alt.Chart(result["data"])
                    .mark_line()
                    .encode(
                        x=alt.X(result["x_column"], title=result["x_column"].upper(),
                                axis=alt.Axis(**axis_cfg)),
                        y=alt.Y(result["y_column"], title=result["y_column"].upper(),
                                axis=alt.Axis(**axis_cfg)),
                        color=alt.value("blue"),
                        tooltip=[result["x_column"], result["y_column"]],
                    )
