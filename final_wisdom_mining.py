...
#         selected_location_table = "province_wise_electricity_consumption_gwh"
#         selected_utility_table = "electricity_consumption_by_sector_gwh"

#         # st.sidebar.title("Below Toggle is to move Between CBNARM or WiseRule")
#         with st.sidebar.expander("Purpose of Use Location & Utility Dimensions Toggle"):
#             st.markdown("""
#             **Toggle Exmplained:**
#             - **If True**: WISERULE will work.
#             - **If False**: CBNARM will work.
#             """)
#         use_dimensions = st.sidebar.checkbox("Use Location & Utility Dimensions", value=True)
...
#                 st.toast("Wisdom Mining Analysis started!", icon="🧠")
...
#                 tabs = st.tabs(["All Rules", "Metric Analysis", "Top Rules", "Smart Analysis"])
...
#                     top_rules = rule_df.head(12).copy()
...
#                 with tabs[3]:
#                     st.subheader("Smart Analysis of WisRule Results")
                    
#                     # Prepare data for analysis
#                     analysis_data = {
#                         "total_rules": len(rule_df),
#                         "positive_rules": len(rule_df[rule_df["Rule Type"] == "Positive"]),
#                         "negative_rules": len(rule_df[rule_df["Rule Type"] == "Negative"]),
#                         "avg_confidence": rule_df["Confidence"].mean(),
#                         "avg_wisval": rule_df["WisVal"].mean(),
#                         "top_rules": top_rules[["Rule", "Rule Type", "Confidence", "WisVal"]].to_dict('records'),
#                         "rule_types_distribution": rule_types.to_dict(),
#                         "confidence_range": {
#                             "min": rule_df["Confidence"].min(),
#                             "max": rule_df["Confidence"].max(),
#                             "std": rule_df["Confidence"].std()
#                         },
#                         "wisval_range": {
#                             "min": rule_df["WisVal"].min(),
#                             "max": rule_df["WisVal"].max(),
#                             "std": rule_df["WisVal"].std()
#                         }
#                     }
                    
#                     # Create analysis prompt
#                     analysis_prompt = f"""
#                     Analyze the following WisRule mining results:

#                     Total Rules: {analysis_data['total_rules']}
#                     Positive Rules: {analysis_data['positive_rules']}
#                     Negative Rules: {analysis_data['negative_rules']}
#                     Average Confidence: {analysis_data['avg_confidence']:.2f}
#                     Average WisVal: {analysis_data['avg_wisval']:.2f}

#                     Top Rules:
#                     {analysis_data['top_rules']}
...
#                     Focus on the most significant patterns and their implications.
#                     """
                    
#                     # Add a button to trigger analysis
#                     if st.button("Generate Smart Analysis"):
#                         st.toast("Smart analysis generated!", icon="✨")
...
#                                 analysis = f"""
#                                 ## Overview
#                                 - Dataset contains {analysis_data['total_rules']} rules with {analysis_data['positive_rules']/analysis_data['total_rules']*100:.1f}% positive rules
#                                 - Rule confidence ranges from {analysis_data['confidence_range']['min']:.2f} to {analysis_data['confidence_range']['max']:.2f}
#                                 - WisVal scores show {analysis_data['wisval_range']['std']:.2f} standard deviation, indicating {('high' if analysis_data['wisval_range']['std'] > 0.2 else 'moderate')} variability in rule quality

#                                 ## Pattern Analysis
#                                 - {analysis_data['positive_rules']} positive rules indicate strong direct relationships
#                                 - {analysis_data['negative_rules']} negative rules reveal important inverse relationships
#                                 - Average confidence of {analysis_data['avg_confidence']:.2f} suggests {('high' if analysis_data['avg_confidence'] > 0.7 else 'moderate')} rule reliability
#                                 - WisVal scores averaging {analysis_data['avg_wisval']:.2f} indicate {('strong' if analysis_data['avg_wisval'] > 0.6 else 'moderate')} pattern significance

#                                 ## Key Insights
#                                 - Top rules show confidence levels ranging from {min(top_rules['Confidence']):.2f} to {max(top_rules['Confidence']):.2f}
#                                 - Rule distribution shows {('balanced' if abs(analysis_data['positive_rules'] - analysis_data['negative_rules']) < 5 else 'skewed')} pattern between positive and negative rules
#                                 - {('High' if analysis_data['avg_wisval'] > 0.6 else 'Moderate')} WisVal scores suggest {('strong' if analysis_data['avg_wisval'] > 0.6 else 'moderate')} predictive power

#                                 ## Future Predictions
#                                 - Based on current patterns, expect {('increasing' if analysis_data['positive_rules'] > analysis_data['negative_rules'] else 'decreasing')} trend in rule strength
#                                 - Confidence levels suggest {('stable' if analysis_data['confidence_range']['std'] < 0.1 else 'variable')} future rule reliability
#                                 - WisVal distribution indicates {('strong' if analysis_data['avg_wisval'] > 0.6 else 'moderate')} potential for future pattern discovery

#                                 ## Recommendations
#                                 - Focus on rules with WisVal > {analysis_data['avg_wisval']:.2f} for most reliable insights
#                                 - Consider investigating negative rules for unexpected relationships
#                                 - Monitor patterns in top rules for trend analysis
#                                 - {('Increase' if analysis_data['avg_confidence'] < 0.7 else 'Maintain')} confidence thresholds for optimal rule quality
#                                 - {('Explore' if analysis_data['negative_rules'] < analysis_data['positive_rules'] else 'Validate')} negative rule patterns for comprehensive analysis
#                                 """
...
#                                 st.download_button(
#                                     label="Download Smart Analysis",
#                                     data=analysis,
#                                     file_name="wisrule_smart_analysis.txt",
#                                     mime='text/plain'
#                                 )
...
def load_wisdom_mining(logger):
    ...
    def fetch_tables() -> list:
        """Return user-created table names via the PostgREST RPC."""
        try:
            url = f"{SUPABASE_URL}/rest/v1/rpc/get_public_tables"
            resp = requests.post(url, headers=_supabase_headers(SUPABASE_KEY), timeout=15)
            ...
            with st.spinner(f'Connecting to database table {table_name}...'):
                ...
                if "Installed Capacity (MW)" in data.columns:
                    data["Installed Capacity (MW)"] = (
                        data["Installed Capacity (MW)"] * 8760
                    ) / 1000
                    data.rename(
                        columns={"Installed Capacity (MW)": "Installed Capacity (GWh)"},
                        inplace=True,
                    )
    ...
    def fetch_table(table_name: str) -> pd.DataFrame:
        """Fetch all rows from an arbitrary table via Supabase REST."""
        try:
            url = f"{SUPABASE_URL}/rest/v1/{table_name}"
            headers = {
                **_supabase_headers(SUPABASE_KEY),
    ...
    class WisRuleWithNegative:
        ...
        def generate_frequent_itemsets(self):
            ...
            for transaction in self.transactions:
                for item in transaction:
                    item_counts[frozenset([item])] = (
                        item_counts.get(frozenset([item]), 0) + 1
                    )

            self.utility_dict["item_counts"] = {
                next(iter(item)): count for item, count in item_counts.items()
            }
            ...
            while current_itemsets:
                new_candidates = [
                    i.union(j)
                    for i in current_itemsets
                    for j in current_itemsets
                    if len(i.union(j)) == k
                ]
                candidate_counts = {candidate: 0 for candidate in new_candidates}
        ...
        def calculate_upii(self, antecedent, consequent, positive=True):
            ...
            if positive:
                joint_prob = (
                    self.utility_dict.get("itemset_counts", {}).get(
                        frozenset(antecedent.union(consequent)), 0
                    )
                    / self.total_transactions
                )
                ...
                negation_count = 0
                for transaction in self.transactions:
                    if antecedent.issubset(transaction) and not consequent.issubset(
                        transaction
                    ):
                        negation_count += 1
                joint_prob = negation_count / self.total_transactions
            ...
            expected_prob = ant_support * cons_support
            return (
                (joint_prob - expected_prob) / max(joint_prob, expected_prob)
                if max(joint_prob, expected_prob) > 0
                else 0
            )
        ...
        def calculate_lift(self, antecedent, consequent, positive=True):
            ...
            return (
                joint_prob / (ant_support * cons_support)
        ...
        def evaluate_rule(self, antecedent, consequent, positive=True):
            ...
            if positive:
                utility = (
                    confidence / consequent_support if consequent_support > 0 else 0
                )
            else:
                utility = (
                    confidence / complement_support if complement_support > 0 else 0
                )
    ...
    def create_dimension_transactions(location_df, utility_df):
        ...
        for i in range(len(location_df)):
            row = []
            location = location_df.iloc[i].get("Province") or location_df.iloc[i].get(
                "Region"
            )
            if location:
                row.append(f"Location: {location}")
    ...
    if st.session_state.df is not None:
        ...
        with st.sidebar.expander("Purpose of Use Location & Utility Dimensions Toggle"):
            ...
            **Toggle Explained:**
            ...
            - **If False**: CBNARM will work.
            """)
        use_dimensions = st.sidebar.checkbox(
            "Use Location & Utility Dimensions", value=True
        )
        ...
        if missing_columns:
            ...
            selected_columns = st.multiselect(
                "Select columns for analysis (need at least 3)",
                st.session_state.df.select_dtypes(include=["number"]).columns,
                max_selections=4,
            )
            if len(selected_columns) < 3:
        ...
        st.session_state.transactions, column_names = create_yoy_transactions(
            st.session_state.df, required_columns
        )
        ...
        if use_dimensions:
            try:
                location_df = fetch_table(selected_location_table)
                utility_df  = fetch_table(selected_utility_table)
                dimension_transactions = create_dimension_transactions(
                    location_df, utility_df
                )
                st.session_state.transactions.extend(dimension_transactions)
                st.success("Dimension-based transactions added.")
            except Exception as e:
        ...
        with col2:
            if st.button(
                "Run Wisdom Mining Analysis", type="primary", use_container_width=True
            ):
                st.toast("Wisdom Mining Analysis started!", icon="🧠")
                st.session_state.run_analysis = True
    ...
    if st.session_state.run_analysis:
        with st.spinner("Running WisRule Algorithm..."):
            ...
            if not rules:
                st.warning(
                    "No rules meet the support and confidence thresholds. "
                ...
                tabs = st.tabs(
                    ["All Rules", "Metric Analysis", "Top Rules", "Smart Analysis"]
                )
                ...
                with tabs[1]:
                    ...
                    with col1:
                        st.subheader("Confidence by Rule Type")
                        confidence_by_type = (
                            rule_df.groupby("Rule Type")["Confidence"]
                            .mean()
                            .reset_index()
                        )
                        st.bar_chart(confidence_by_type.set_index("Rule Type"))
                    with col2:
                        st.subheader("WisVal by Rule Type")
                        wisval_by_type = (
                            rule_df.groupby("Rule Type")["WisVal"].mean().reset_index()
                        )
                        st.bar_chart(wisval_by_type.set_index("Rule Type"))

                    st.subheader("Comparison of Metrics")
                    metric_df = rule_df[
                        ["Rule Type", "Confidence", "Utility", "Lift", "UPII", "WisVal"]
                    ].melt(id_vars=["Rule Type"], var_name="Metric", value_name="Value")
                    st.line_chart(
                        metric_df.groupby(["Rule Type", "Metric"])["Value"]
                        .mean()
                        .unstack()
                    )
                ...
                with tabs[2]:
                    ...
                    st.dataframe(
                        top_rules[["Rule", "Rule Type", "Confidence", "WisVal"]]
                    )
                    chart_data = top_rules[["Rule", "Confidence", "WisVal"]].set_index(
                        "Rule"
                    )
                    st.bar_chart(chart_data)
                ...
                with tabs[3]:
                    ...
                    analysis_data = {
                        "total_rules": len(rule_df),
                        "positive_rules": len(
                            rule_df[rule_df["Rule Type"] == "Positive"]
                        ),
                        "negative_rules": len(
                            rule_df[rule_df["Rule Type"] == "Negative"]
                        ),
                        "avg_confidence": rule_df["Confidence"].mean(),
                        "avg_wisval": rule_df["WisVal"].mean(),
                        "top_rules": top_rules[
                            ["Rule", "Rule Type", "Confidence", "WisVal"]
                        ].to_dict("records"),
                        "rule_types_distribution": rule_types.to_dict(),
                        "confidence_range": {
                            "min": rule_df["Confidence"].min(),
...
- {('Explore' if analysis_data['negative_rules'] < analysis_data['positive_rules'] else 'Validate')} negative rule patterns for comprehensive analysis
                                ...
                                st.error(f"Error generating analysis: {str(e)}")
                                st.error(
                                    "Please try again or check your LLM service configuration."
                                )
    else:
        st.info("Please Load data to begin analysis")